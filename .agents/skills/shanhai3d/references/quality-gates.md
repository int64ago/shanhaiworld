...
- 在最终 1920×1080 页面镜头量化屏幕可读性：攻击、展示、闪避等动作的主要末端或轮廓峰值通常至少跨越神兽屏幕包围盒的 8%；移动动作的足、翼、鳍或连续推进波通常至少达到 4%。特殊解剖若不宜使用该阈值，必须记录不会伤害连接/体积的等价可见性依据，不能以“已有轨道”代替。
- 移动动作必须看得出接触或推进机制：脚步需要离地、摆动、承重和落点，飞行需要明显上拍/下拍与收腿或身体稳定，游泳/爬行需要鳍肢、躯干波或附肢支撑。世界位移、阴影移动或整体漂浮不构成该证据。
...
- 固定曝光后以 45° 间隔检查 actor 的 8 个朝向；正面、背面和侧面都必须读到脸/头部方向、主要附肢连接、轮廓与中尺度材质。场景必须同时提供主光、补光和轮廓光；使用等距背景时还需把背景纹理用于环境照明。任一朝向大面积死黑、主体与背景粘连或材质 relief 消失都阻断发布。
- 所有 ready 页面使用原生 2:1 latitude-longitude 等距全景和不受限的 OrbitControls 水平方位；禁止把 16:9 平面图拉伸或固定在屏幕上伪装 360° 场景。至少保存四个连续拖拽方位，检查远景随相机连续变化、地面/道具保持世界锁定，并量化或目视验证左右边缘无接缝。
